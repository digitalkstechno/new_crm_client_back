...
exports.updateLead = async (req, res) => {
  try {
    ...
    if (newStatus) {
      // Validate lostDescription when moving to Lost status
      if (newStatus === "Lost" && !req.body.lostDescription) {
        return res.status(400).json({
          status: "Fail",
          message: "Description is required when marking lead as Lost",
        });
      }
